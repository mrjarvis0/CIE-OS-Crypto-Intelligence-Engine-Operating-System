...
the interpreter checks. ``monitoring.metrics`` **is** ``telemetry.metrics`` -- the same
...
def __getattr__(name: str) -> Any:
    """
    Forward any other name to :mod:`telemetry.metrics`.

    A live delegation rather than a star-import: ``from monitoring import X``
    reads ``telemetry.metrics.X`` at the moment of the call, so this package cannot
    ...
    try:
        ...
        raise AttributeError(
            f"module 'monitoring' has no attribute {name!r}; it redirects to "
            f"'telemetry.metrics', which has no such name"
        ) from None


def __dir__() -> list[str]:
    return sorted(set(globals()) | set(dir(metrics)))
