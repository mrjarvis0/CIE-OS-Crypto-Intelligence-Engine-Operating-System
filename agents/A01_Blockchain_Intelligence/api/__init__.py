...
The HTTP surface: read-only, GET-only, loopback-bound by default.
...
the interpreter checks. ``api.rest`` **is** ``interfaces.rest`` -- the same
...
def __getattr__(name: str) -> Any:
    """
    Forward any other name to :mod:`interfaces.rest`.

    A live delegation rather than a star-import: ``from api import X``
    reads ``interfaces.rest.X`` at the moment of the call, so this package cannot
    ...
    try:
        ...
        raise AttributeError(
            f"module 'api' has no attribute {name!r}; it redirects to "
            f"'interfaces.rest', which has no such name"
        ) from None


def __dir__() -> list[str]:
    return sorted(set(globals()) | set(dir(rest)))
