...
the interpreter checks. ``plugins.plugins`` **is** ``tools.plugins`` -- the same
...
def __getattr__(name: str) -> Any:
    """
    Forward any other name to :mod:`tools.plugins`.

    A live delegation rather than a star-import: ``from plugins import X``
    reads ``tools.plugins.X`` at the moment of the call, so this package cannot
    ...
    try:
        ...
        raise AttributeError(
            f"module 'plugins' has no attribute {name!r}; it redirects to "
            f"'tools.plugins', which has no such name"
        ) from None


def __dir__() -> list[str]:
    return sorted(set(globals()) | set(dir(plugins)))
