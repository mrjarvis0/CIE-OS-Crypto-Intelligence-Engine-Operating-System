...
Report rendering. The layer-level package wins, per
...
the interpreter checks. ``reporting.reporting`` **is** ``intelligence.reporting`` -- the same
...
from intelligence import reporting
...
def __getattr__(name: str) -> Any:
    """
    Forward any other name to :mod:`intelligence.reporting`.

    A live delegation rather than a star-import: ``from reporting import X``
    reads ``intelligence.reporting.X`` at the moment of the call, so this package cannot
    ...
    try:
        ...
        raise AttributeError(
            f"module 'reporting' has no attribute {name!r}; it redirects to "
            f"'intelligence.reporting', which has no such name"
        ) from None


def __dir__() -> list[str]:
    return sorted(set(globals()) | set(dir(reporting)))
