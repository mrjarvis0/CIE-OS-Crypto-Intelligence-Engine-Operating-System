...
Reasoning strategies. The layer-level package wins, per
folder-architecture section 11.
...
the interpreter checks. ``reasoning.reasoning`` **is** ``intelligence.reasoning`` -- the same
...
from intelligence import reasoning
...
def __getattr__(name: str) -> Any:
    """
    Forward any other name to :mod:`intelligence.reasoning`.

    A live delegation rather than a star-import: ``from reasoning import X``
    reads ``intelligence.reasoning.X`` at the moment of the call, so this package cannot
    ...
    try:
        ...
        raise AttributeError(
            f"module 'reasoning' has no attribute {name!r}; it redirects to "
            f"'intelligence.reasoning', which has no such name"
        ) from None


def __dir__() -> list[str]:
    return sorted(set(globals()) | set(dir(reasoning)))
