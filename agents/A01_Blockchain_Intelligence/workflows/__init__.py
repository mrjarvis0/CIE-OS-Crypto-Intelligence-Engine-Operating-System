...
the interpreter checks. ``workflows.planning`` **is** ``planning`` -- the same
...
def __getattr__(name: str) -> Any:
    """
    Forward any other name to :mod:`planning`.

    A live delegation rather than a star-import: ``from workflows import X``
    reads ``planning.X`` at the moment of the call, so this package cannot
    ...
    try:
        ...
        raise AttributeError(
            f"module 'workflows' has no attribute {name!r}; it redirects to "
            f"'planning', which has no such name"
        ) from None


def __dir__() -> list[str]:
    return sorted(set(globals()) | set(dir(planning)))
